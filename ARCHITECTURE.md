...
- All source videos are re-encoded for seek.
- L3/L4 pre-concatenates intro, title card background, scene videos, key learnings background, and outro into one generated visual base at `.jobs/<jobId>/assets/l3l4-visual-base.mp4` before HyperFrames renders.
- The single visual base is rendered as one silent video clip; original source-video audio is preserved as separate timeline audio clips at each segment's start/duration.
- Title text, OST chips, key learnings, and logo remain independent overlay clips over the visual base.
...
Key learnings overlays are shared by both strategies. L1/L2 adds a normal video clip for the background and a `keyLearnings` overlay clip on top of it. L3/L4 includes the key learnings background inside its generated visual base and adds the `keyLearnings` overlay clip on top. The background video duration controls the overlay duration, and the clip is placed after all scenes and before outro.
...
These video paths emit 30fps H.264 with GOP/keyframe settings of `-g 30 -keyint_min 30` so HyperFrames can seek reliably during frame capture. `concatenateTimelineVideos` also normalizes the visual base to yuv420p and the target composition dimensions.
...
There is no post-render intro/outro stitching. Intro and outro are prepared before rendering and remain part of the generated HyperFrames timeline (for L3/L4, inside the pre-concatenated visual base).
